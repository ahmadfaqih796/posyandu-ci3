      ...
      <div class="modal-content">
         ...
         <form action="<?= base_url('management/anak') ?>" method="post">
            <div class="modal-body">
               ...
               <div class="form-group">
                  <label for="edit_orang_tua_id">Nama Orang Tua</label>
                  <select name="orang_tua_id" id="edit_orang_tua_id" class="form-control">
                     <?php foreach ($ibu as $field) : ?>
                        <option value="<?= $field['id'] ?>" <?= set_select('orang_tua_id', $field['id'], (!empty($_POST['orang_tua_id']) && $_POST['orang_tua_id'] == $field['id'])); ?>><?= $field['n_ibu'] ?></option>
                     <?php endforeach; ?>
                  </select>
                  <?= form_error('jk', '<small class="text-danger pl-3">', '</small>'); ?>
               </div>
...
<script>
   function getData(data) {
      ...
      document.getElementById('edit_orang_tua_id').value = data['orang_tua_id'];
